...
void exec_while(const STRING line) {
	...
	if (!stack_while[top].isrun) {
		...
		if (cp >= code_size) {
			fprintf(stderr, "WHILE 没有对应的 WEND\n");
			exit(EXIT_FAILURE);
		} else {
			cp--;
		}
...
void exec_wend(const STRING line) {
	if (top_while < 0) {
		fprintf(stderr, "WEND 没有对应的 WHILE\n");
		exit(EXIT_FAILURE);
...
void exec_if(const STRING line) {
	...
	if (!stack_if[top]) {
		...
		if (cp >= code_size) {
			fprintf(stderr, "IF 没有对应的 END IF\n");
			exit(EXIT_FAILURE);
		} else {
			cp--;
		}
...
void exec_endif(const STRING line) {
	if (top_if < 0) {
		fprintf(stderr, "END IF 没有对应的 IF\n");
		exit(EXIT_FAILURE);
